"""Build the 2-min video from the ACTUAL exported PPTX slides + same voiceover. ~119s."""
import json, os, subprocess
import imageio_ffmpeg

HERE = os.path.dirname(__file__)
VO = os.path.join(HERE, "video", "vo")
SLIDES = os.path.join(HERE, "pptx_slides")
...
n = len(durs)
GAP = (TARGET - sum(durs)) / n
assert GAP > 0, f"narration too long ({sum(durs):.1f}s)"
print(f"speech={sum(durs):.1f}s  gap/scene={GAP:.2f}s")

VF = ("scale=1920:1080:force_original_aspect_ratio=decrease,"
      "pad=1920:1080:(ow-iw)/2:(oh-ih)/2:color=white,setsar=1,format=yuv420p,fps=30")
...
for i, m in enumerate(meta):
    sd = durs[i] + GAP
    img = os.path.join(SLIDES, f"Slide{i+1}.PNG")
    mp3 = os.path.join(VO, m["name"] + ".mp3")
    out = os.path.join(VO, f"pscene{i+1}.mp4")
    run([FF, "-y", "-loop", "1", "-framerate", "30", "-t", f"{sd:.3f}", "-i", img,
         ...
         "-t", f"{sd:.3f}", out])
    scenes.append(out)
    print(f"scene{i+1}: {sd:.2f}s  <- Slide{i+1}.PNG")

listfile = os.path.join(VO, "pconcat.txt")
